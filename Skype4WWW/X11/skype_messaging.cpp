...
// #		else /*if !apple */
// #			ifndef SKYPE_DBUS
// #				include "skype_messaging_x11.cpp"
// #			else 
...
void skype_message_received(char *orig_message)
{
	...
	if(message[0] == '#')
	{
		...
		*key = request_number;
        ...
        fprintf(stderr, "skype message: %s\n", (char*)message);
        iret = skype_handle_received_message(message);
	}
}
...
send_messages_thread_func(gpointer data)
{
	...
	while (send_thread_state)
	{
		...
		message = (gchar*)g_async_queue_pop(send_messages_queue);
		//send the message
...
char *skype_send_message(char *message_format, ...)
{
    ...
    fprintf(stderr, "here msg to send: #%u %s\n", cur_message_num, message);
