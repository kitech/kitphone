...
class YStatusBarPrivate : public QWidgetPrivate
{
    Q_DECLARE_PUBLIC(YStatusBar)
public:
    YStatusBarPrivate() {}
...
QRect YStatusBarPrivate::messageRect() const
{
    Q_Q(const YStatusBar);
    ...
    for (int i=items.size()-1; i>=0; --i) {
        YStatusBarPrivate::SBItem* item = items.at(i);
        if (!item)
            break;

        // qDebug()<<item->w<<item->p<<item->w->geometry()<<item->w->x()<<item->w->width()<<left<<right<<rtl
        //         <<item->w->isVisible();
        if (!item->p){
            if (rtl) {
                ...
                // qDebug()<<"gooooooooooo:"<<left;
            }
            break;
        }
    }
    // qDebug()<<left<<(right-left)<<q->height();
...
/*!
    \class YStatusBar
    \brief The YStatusBar class provides a horizontal bar suitable for
    ...
    YStatusBar lets you display all three types of indicators.
    ...
    message currently shown. The YStatusBar class also provide the
    ...
    By default YStatusBar provides a QSizeGrip in the lower-right
...
/*!
    ...
    Use the YStatusBar() constructor and the QObject::setObjectName()
    function instead.

    \oldcode
        YStatusBar *myStatusBar = new YStatusBar(parent, name);
    \newcode
        YStatusBar *myStatusBar = new YStatusBar(parent);
        myStatusBar->setObjectName(name);
    \endcode
*/
YStatusBar::YStatusBar(QWidget * parent, const char *name)
    : QWidget(*new YStatusBarPrivate, parent, 0)
{
    Q_D(YStatusBar);
...
/*!
    ...
    widget if it isn't already a child of this YStatusBar object. The
...
void YStatusBar::addWidget(QWidget * widget, int stretch)
{
    if (!widget)
        return;
...
/*!
    ...
    YStatusBar object. If \a index is out of range, the widget is appended
...
*/
int YStatusBar::insertWidget(int index, QWidget *widget, int stretch)
{
    if (!widget)
        return -1;

    Q_D(YStatusBar);
    YStatusBarPrivate::SBItem* item = new YStatusBarPrivate::SBItem(widget, stretch, false);

    int idx = d->indexToLastNonPermanentWidget();
    if (index < 0 || index > d->items.size() || (idx >= 0 && index > idx + 1)) {
        qWarning("YStatusBar::insertWidget: Index out of range (%d), appending widget", index);
        index = idx + 1;
    }
...
/*!
    ...
    reparenting the widget if it isn't already a child of this
    YStatusBar object. The \a stretch parameter is used to compute a
...
void YStatusBar::addPermanentWidget(QWidget * widget, int stretch)
{
    if (!widget)
        return;
...
*/
int YStatusBar::insertPermanentWidget(int index, QWidget *widget, int stretch)
{
    if (!widget)
        return -1;

    Q_D(YStatusBar);
    YStatusBarPrivate::SBItem* item = new YStatusBarPrivate::SBItem(widget, stretch, true);

    int idx = d->indexToLastNonPermanentWidget();
    if (index < 0 || index > d->items.size() || (idx >= 0 && index <= idx)) {
        qWarning("YStatusBar::insertPermanentWidget: Index out of range (%d), appending widget", index);
        index = d->items.size();
    }
    d->items.insert(index, item);
...
void YStatusBar::removeWidget(QWidget *widget)
{
    if (!widget)
        return;
    ...
    if (found)
        ...
        qDebug("YStatusBar::removeWidget(): Widget not found.");
#endif
}

/*!
    \property YStatusBar::sizeGripEnabled
...
bool YStatusBar::isSizeGripEnabled() const
{
#ifdef QT_NO_SIZEGRIP
    return false;
#else
    Q_D(const YStatusBar);
    return !!d->resizer;
#endif
}

void YStatusBar::setSizeGripEnabled(bool enabled)
{
#ifdef QT_NO_SIZEGRIP
    Q_UNUSED(enabled);
#else
...
*/
void YStatusBar::showMessage(const QString &message, int timeout)
{
    Q_D(YStatusBar);
...
/*!
    ...
    \sa showMessage()
*/
QString YStatusBar::currentMessage() const
{
    Q_D(const YStatusBar);
    return d->tempItem;
}
...
/*!
    \fn void YStatusBar::message(const QString &message, int timeout)

    Use the showMessage() function instead.
*/

/*!
    \fn void YStatusBar::clear()

    Use the clearMessage() function instead.
*/

/*!
    \fn YStatusBar::messageChanged(const QString &message)
...
*/
void YStatusBar::hideOrShow()
{
    Q_D(YStatusBar);
    bool haveMessage = !d->tempItem.isEmpty();

    YStatusBarPrivate::SBItem* item = 0;
    for (int i=0; i<d->items.size(); ++i) {
        item = d->items.at(i);
        if (!item || item->p)
            break;
        if (haveMessage && item->w->isVisible()) {
            ...
            // item->w->setAttribute(Qt::WA_WState_ExplicitShowHide, true);
...
/*!
  \reimp
 */
void YStatusBar::showEvent(QShowEvent *)
{
#ifndef QT_NO_SIZEGRIP
    Q_D(YStatusBar);
...
/*!
    \reimp
    \fn void YStatusBar::paintEvent(QPaintEvent *event)

    Shows the temporary message, if appropriate, in response to the
    paint \a event.
*/
void YStatusBar::paintEvent(QPaintEvent *event)
{
    Q_D(YStatusBar);
    bool haveMessage = !d->tempItem.isEmpty();
    ...
    for (int i=0; i<d->items.size(); ++i) {
        YStatusBarPrivate::SBItem* item = d->items.at(i);
        // qDebug()<<item<<item->w<<item->w->isVisible()<<haveMessage<<item->p<<item->w->geometry();
...
/*!
    \reimp
*/
void YStatusBar::resizeEvent(QResizeEvent * e)
{
...
bool YStatusBar::event(QEvent *e)
{
    Q_D(YStatusBar);
