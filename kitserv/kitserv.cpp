...
int serv_instance_main(int argc, char **argv, int way_id)
{
    ...
    for (int i = 0 ; i < way_count ; ++i) {
        ...
        ctxes[way_id] = ctx = new ServContex(way_id, storage_dir, switcher_skype_name.toAscii().data());
        ctx->m_db = db;
 ...
 */
///////////////////////
ServContex::ServContex(int ctx_id, char *storage_dir, char *skype_name) 
    : m_ctx_id(ctx_id)
{
    this->m_db = NULL;
...
void ServContex::onNewForwardCallArrived(SEReference &conversation, QString callerName, QString calleeName, int skypeCallID)
{
    ...
    // 查找 call pair
    callee_phone = this->m_db->getCallPeer(callerName, caller_ipaddr);
