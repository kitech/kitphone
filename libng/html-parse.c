...
static void
nullget_link_mapper (struct taginfo *taginfo, void *arg)
{
	int i;	
	char * now_link = 0 ; 	

	struct nullget_tag_mapper_arg * sarg = (struct nullget_tag_mapper_arg *) arg ;	

	if( strcasecmp(taginfo->name,"a") == 0 )
	{
		for (i = 0; i < taginfo->nattrs; i++)
		{
			printf (" %s=%s\n", taginfo->attrs[i].name, taginfo->attrs[i].value); 
			if( strcasecmp(taginfo->attrs[i].name,"href") == 0 && taginfo->attrs[i].value != 0 )
			{
				if( sarg->link == 0 )
				{
					sarg->link = (char **) malloc(sizeof(char**));
				}
				else
				{
					sarg->link = (char**)realloc( sarg->link ,(sarg->count+1)*sizeof(char**) );
				}
				now_link = (char*) malloc(strlen(taginfo->attrs[i].value)+1);
				strcpy(now_link,taginfo->attrs[i].value);
				now_link[strlen(taginfo->attrs[i].value)] = '\0';
				sarg->link[sarg->count] = now_link ;
				sarg->count ++ ;
...
char ** html_parse_get_all_link(const char * htmlcode , int * count )
{
	int size = 256;
	char *x = 0 ;
	int length = 0;
	...
	char ** link = 0 ;
	struct nullget_tag_mapper_arg arg ;
	arg.count =0 ;
	arg.link = 0 ;

	if( htmlcode == 0 ) return 0 ;

	length = strlen(htmlcode);
	x = htmlcode ;

	map_html_tags ( x, length , nullget_link_mapper , (void*)&arg , 0 , NULL, NULL);
	printf ("TAGS: %d\n", arg.count);
	printf ("Tag backouts:     %d\n", tag_backout_count);
	printf ("Comment backouts: %d\n", comment_backout_count);
...
void html_parse_free_link_mt(char ** link , int count)
{
	int i ;
	for( i = 0 ; i < count ; ++i )
	{
...
static void
nullget_image_mapper (struct taginfo *taginfo, void *arg)
{
	int i;
	char * now_link = 0 ; 	
	struct nullget_tag_mapper_arg * sarg = (struct nullget_tag_mapper_arg *) arg ;

	if( strcasecmp(taginfo->name,"img") == 0 )
	{
		for (i = 0; i < taginfo->nattrs; i++)
		{
			printf (" %s=%s\n", taginfo->attrs[i].name, taginfo->attrs[i].value); 
			if( strcasecmp(taginfo->attrs[i].name,"src") == 0 && taginfo->attrs[i].value != 0 )
			{
				if( sarg->link == 0 )
				{
					sarg->link = (char **) malloc(sizeof(char**));
...
char ** html_parse_get_all_image(const char * htmlcode , int * count )
{
	...
	map_html_tags (x, length , nullget_image_mapper , (void*)&arg, 0, NULL, NULL);
	printf ("TAGS: %d\n", arg.count);
	printf ("Tag backouts:     %d\n", tag_backout_count);
	printf ("Comment backouts: %d\n", comment_backout_count);
