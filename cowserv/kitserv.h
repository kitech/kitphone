...
class ServContex {
public:
    ServContex(int ctx_id, int ws_fd, char *storage_dir, char *skype_name);
    ~ServContex();

    int connect_skype_runtime(char *app_token, unsigned short app_port, QString skype_name, QString password);
    int connect_skype_runtime(char *app_token, unsigned short app_port); // connect but not login
    int login_skype_runtime(QString skype_name, QString password);
    ...
    int init_websocket_connection_thread();
    ...
    KBiHash<int, QString> m_websocket_client_fds; // websocket fd => skype name
    KBiHash<int, void *> m_websocket_client_ws_ctxes; // websocket fd => websocket ctx

    pthread_t m_websock_connection_thread;
